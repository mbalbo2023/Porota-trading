...
class NominalEvidence:
    ...
    quote_basis_nominal:object|None; quantity_step_nominal:object|None; minimum_nominal:object|None
...
class NominalContract:
    ...
    metadata_source:str; observed_at:str; source_reference:str
    status:str='VERIFIED_NOMINAL_CONTRACT'; paper_execution_authorized:bool=False
    ...
    def cash_notional(self,price,nominal)->Decimal:
        p=_d(price,'PRICE'); q=self.normalize_quantity(nominal); return p*q/self.quote_basis_nominal
def evaluate(e:NominalEvidence)->dict:
    ...
    if family not in FAMILIES or any(x in {'','UNKNOWN'} for x in identity): return {'status':'IDENTITY_UNVERIFIED','contract':None,'paper_execution_authorized':False}
    if not str(e.metadata_source or '').strip() or not str(e.observed_at or '').strip(): return {'status':'PROVENANCE_INCOMPLETE','contract':None,'paper_execution_authorized':False}
    missing=[name for name,value in [('quote_basis_nominal',e.quote_basis_nominal),('quantity_step_nominal',e.quantity_step_nominal),('minimum_nominal',e.minimum_nominal)] if value is None]
    if missing: return {'status':'NEEDS_NOMINAL_UNITS','missing':missing,'contract':None,'paper_execution_authorized':False}
    try: basis=_d(e.quote_basis_nominal,'QUOTE_BASIS_NOMINAL'); step=_d(e.quantity_step_nominal,'QUANTITY_STEP_NOMINAL'); minimum=_d(e.minimum_nominal,'MINIMUM_NOMINAL')
