...
def test_option_needs_multiplier_even_when_chain_completes_other_fields():
    ...
    assert r["paper_simulatable"] is False
...
def test_complete_option_contract_still_waits_for_specialized_simulator():
    rows=[
      {"source_class":"PPI_STRUCTURED_API","evidence":{"market":"BYMA","currency":"ARS","settlement":"INMEDIATA"}},
      {"source_class":"IOL_MCP","evidence":{"underlying":"GGAL","put_call":"CALL","strike":4200,
       "expiry_at":"2026-10-16T15:30:00-03:00","quantity_step":1,"contract_multiplier":100}},
    ]
    r=c.readiness("OPCIONES",rows)
    assert r["contract_complete"] is True
    assert r["status"]=="CONTRACT_READY_SIMULATOR_PENDING"
    assert r["paper_simulatable"] is False
