...
def test_refresh_stays_shadow_and_cannot_change_live_authority(tmp_path):
    def tool(name, args):
        return {"last": 100, "bid": 99, "ask": 101} if name == "get_asset_quote" else {
            "type": "ACCIONES", "currency": "ARS", "units_per_lot": 1}

    data = shadow.refresh(["YPFD"], tool, root=tmp_path,
                          primary_last_by_symbol={"YPFD": 96}, tolerance_pct=2)
...
def test_collect_is_cache_only_and_nonblocking(tmp_path):
    ...
    shadow.refresh(["GGAL"], lambda name, args: (
        {"last": 100} if name == "get_asset_quote" else {"type": "ACCIONES"}), root=tmp_path)
