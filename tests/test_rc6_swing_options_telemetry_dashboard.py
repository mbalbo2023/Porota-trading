...
def test_final_routes_keep_eod_and_add_shadow_sections(monkeypatch):
    ...
    try:
        monkeypatch.setattr(
            ...
            "trading_page",
            lambda section="": (
                "<main><section id='rc6-eod-overnight-final-route'>"
...
def test_scalping_telemetry_remains_best_effort_and_threshold_contract_unchanged():
    ...
    policy = Path("fg_intraday_contract_policy_rc6.py").read_text(encoding="utf-8")
    assert "DEFAULT_MUTABLE_SECONDS = 120" in policy
