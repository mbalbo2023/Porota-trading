...
def test_complete_explicit_contract_computes_dimensional_notional():
    r=m.evaluate(ev()); assert r['status']=='VERIFIED_NOMINAL_CONTRACT'
    c=r['contract']; assert c.cash_notional(80000,100)==Decimal('80000')
    assert r['paper_execution_authorized'] is False

def test_missing_minimum_stays_needs_nominal_units():
    r=m.evaluate(ev(minimum_nominal=None)); assert r['status']=='NEEDS_NOMINAL_UNITS'
    assert 'minimum_nominal' in r['missing']; assert r['contract'] is None

def test_iol_units_per_lot_alone_never_claims_full_contract():
    e=m.evidence_from_iol_asset(symbol='GD30',family='BONOS',market='BYMA',currency='ARS',settlement='A-24HS',units_per_lot=100,observed_at='2026-09-07T17:00:00+00:00')
    r=m.evaluate(e); assert r['status']=='NEEDS_NOMINAL_UNITS'
    assert set(r['missing'])=={'quote_basis_nominal','minimum_nominal'}

def test_minimum_must_be_multiple_of_step():
    r=m.evaluate(ev(quantity_step_nominal=100,minimum_nominal=150)); assert r['status']=='INVALID_NOMINAL_CONTRACT'
...
def test_unknown_identity_is_fail_closed():
    assert m.evaluate(ev(market='UNKNOWN'))['status']=='IDENTITY_UNVERIFIED'
