...
PROOF='https://trading.portfoliopersonal.com/Account/Seguridad/DispositivosVinculados?tab=1'
...
def security_page_proof(page):
    try:
        page.goto(PROOF,wait_until='domcontentloaded',timeout=15000); page.wait_for_timeout(1200)
    except Exception:
        return False
    u=urlsplit(page.url); txt=body(page).lower()
    return u.netloc=='trading.portfoliopersonal.com' and '/account/seguridad/dispositivosvinculados' in u.path.lower() and 'dispositivos vinculados' in txt and 'seguros' in txt
    ...
    with sync_playwright() as pw:
        ...
        def onresp(resp):
            try:
                ...
                if u.netloc in PPI_HOSTS and len(first_party)<60:
                    first_party.append(f'{r.method.upper()} {u.netloc}{u.path} -> {resp.status}'[:220])
            except Exception: pass
        page.on('response',onresp)
        if security_page_proof(page):
            ctx.close(); print(json.dumps({'status':'AUTHENTICATED_TRUSTED_DEVICE','proof':'SECURITY_PAGE','submit_count':0,'first_party':first_party[-20:],'blocked_first_party':blocked_first_party[-10:],'credentials_exposed':False,'real_orders_sent':0},sort_keys=True)); return 0
        ...
        if not (un and pwf and sub):
            ctx.close(); print(json.dumps({'status':'BLOCKED_LOGIN_FORM_SHAPE','username_field':bool(un),'password_field':bool(pwf),'submit':bool(sub),'first_party':first_party[-20:],'blocked_first_party':blocked_first_party[-10:],'credentials_exposed':False,'real_orders_sent':0},sort_keys=True)); return 4
        un.fill(user); pwf.fill(pwd); sub.click(no_wait_after=True,timeout=8000); page.wait_for_timeout(3500)
        txt=body(page)
        if TRUST.search(txt) and NOW_NOT.search(txt):
            ...
            if skip is not None:
                before=len(blocked_first_party); skip.click(no_wait_after=True,timeout=8000); page.wait_for_timeout(1800)
                if len(blocked_first_party)>before:
                    ctx.close(); print(json.dumps({'status':'BLOCKED_TRUST_DEVICE_FIRST_PARTY_POST','submit_count':1,'blocked_first_party':blocked_first_party[-10:],'credentials_exposed':False,'real_orders_sent':0},sort_keys=True)); return 4
        if security_page_proof(page):
            ctx.close(); print(json.dumps({'status':'AUTHENTICATED_TRUSTED_DEVICE','proof':'SECURITY_PAGE_AFTER_LOGIN','submit_count':1,'first_party':first_party[-20:],'blocked_first_party':blocked_first_party[-10:],'credentials_exposed':False,'real_orders_sent':0},sort_keys=True)); return 0
        txt=body(page); u=urlsplit(page.url); status='AUTH_STAYED_ON_LOGIN_UNCLASSIFIED'
        if OTP.search(txt): status='AUTH_OTP_REQUIRED'
        ...
        out={'status':status,'submit_count':1,'page_host':u.netloc,'page_path':u.path,'first_party':first_party[-20:],'blocked_first_party':blocked_first_party[-10:],'credentials_exposed':False,'real_orders_sent':0}
        ctx.close(); print(json.dumps(out,sort_keys=True)); return 4
