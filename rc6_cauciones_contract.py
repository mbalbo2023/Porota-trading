...
_MARKET_OPEN = time(10, 30); _MARKET_CLOSE = time(17, 0)
class CaucionContractError(ValueError): pass
@dataclass(frozen=True)
class CaucionIdentity: ticker:str; currency_prefix:str; term_days:int
@dataclass(frozen=True)
class CaucionQuote: ticker:str; tna_percent:Decimal; quantity:Decimal; side_source:str
@dataclass(frozen=True)
class CaucionCostEstimate:
    gross_interest:Decimal; ppi_commission:Decimal; iva_on_ppi_commission:Decimal
    net_before_market_rights_and_other_taxes:Decimal; market_rights_known:bool; budget_is_authoritative_for_full_costs:bool
def parse_ticker(ticker:str)->CaucionIdentity:
    ...
    prefix,days_raw=match.groups(); return CaucionIdentity(ticker=raw,currency_prefix=prefix,term_days=int(days_raw))
...
def theoretical_liquidity_date(order_load_date:date,ticker:str)->date: return order_load_date+timedelta(days=parse_ticker(ticker).term_days)
def is_same_day_concertation_window(moment:datetime)->bool:
    ...
    local=moment.astimezone(_ARGENTINA_TZ).timetz().replace(tzinfo=None); return _MARKET_OPEN<=local<=_MARKET_CLOSE
...
def estimate_ppi_costs_before_market_rights(ticker:str,capital:Any,tna_percent:Any,*,ppi_commission_annual_percent:Any,iva_percent:Any)->CaucionCostEstimate:
    ...
    gross=gross_interest(capital_d,tna_percent,identity.term_days); commission=gross_interest(capital_d,commission_pct,identity.term_days); iva=commission*iva_pct/Decimal("100")
