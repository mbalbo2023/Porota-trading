...
## Wave A — base auditada; WA-01 y WA-03 implementados en PRs aislados
...
Slices de Wave A sobre la rama de código RC6 aislada: WA-01 resolver puro está en PR borrador #63 con CI verde. Hallazgo WA-02 (auditoría estática de `cp_contract_evidence_v2_hf6.py`, blob `2a2eed9843a024ff29fc39022b6b38094be9b4f8`): Evidence v2 keyea por family/ticker/market/settlement/source_class; omite venue/currency/subfamily; current PK y snapshots pueden colisionar; `current_records()` invoca `init_schema()` (DDL), así que no sirve como vista estrictamente read-only ni como identidad canónica sin colisiones. La vía segura a diseñar es vista en memoria por SELECT en conexión `mode=ro`/`query_only`, con `IDENTITY_INCOMPLETE` y `LEGACY_KEY_COLLISION` en HOLD; snapshots no se promocionan sin regla de latest-per-expanded-key. No se consultó DB. Después: allowlist IOL read-only; PPI DTO; evaluator/readiness; divergencia. Schema design antes de persistir; sin migraciones ahora.
...
- CI de WA-01: run final `34887025560` SUCCESS en head `3d4459efd0fd617a8e736b63c80e509b76366f19`; compileall y 20 pruebas offline pasaron. El run `34886924059` falló una expectativa: un registro sin provenance por fila debe ser `CONFLICT`, no `INSUFFICIENT`; se corrigió y el final quedó verde. Valida sólo el slice aislado.
- WA-02 no debe usar `current_records()` ni escribir en Evidence v2 actual. Diseñar/validar primero la vista read-only ampliada y sus colisiones; no se inspeccionó ni modificó DB.
- WA-03: PR borrador #64, wrapper IOL read-only con 10 pruebas offline; Actions run 34888608698 SUCCESS en head 3f634b5b0066e752cfd64b9be646a1adb00dff56. El wrapper no valida payload ni calcula freshness/readiness y sigue sin integrarse.
- Pendiente de Wave A: ratificar taxonomía canónica; WA-02 proyección read-only, WA-04 DTO PPI, WA-05 readiness, WA-06 divergencias, WA-07 diseño de persistencia y wiring del gate de apertura PAPER.
...
- 🟡 Wave A: base aislada correcta; WA-01 (#63, 20 pruebas) y WA-03 (#64, 10 pruebas) tienen CI verde en PRs borrador. WA-02/04/05/06/07 y wiring pendientes; los slices no están integrados al branch de integración ni conectados al runtime.
...
3. 🟡 WA-01/02: PR #63 implementa WA-01; CI final verde con 20 tests. Requiere subfamily y provider ID por fila, underlying para CEDEAR, dimensiones de serie en opciones/futuros y settlement canónico cerrado. Pendiente integrar/revisar el slice y ratificar vocabulario. Para provenance, Evidence v2 no es clave segura: falta venue/currency/subfamily y `current_records()` ejecuta DDL; primero diseñar la proyección read-only con detección de colisiones, sin migración.
4. 🟡 WA-03 quedó implementado en PR #64 con 10 tests offline y CI verde; WA-04 sigue pendiente como DTO PPI sobre normalizador existente, con fixtures sanitizados/mocks y sin red ni credenciales.
...
7. 🟡 WA-07: diseñar el key/schema ampliado o sidecar con venue/currency/subfamily y reglas de historial/colisión; es prerequisito para persistencia WA-02. En esta ola sólo diseño, sin DDL, migración ni backfill.
...
## Handoff de ejecución — actualización 2026-09-14 19:45 UTC

- WA-03: PR borrador [#64](https://github.com/mbalbo2023/Porota-trading/pull/64), rama feature/wave-a-iol-readonly-20260914, aislada sobre integración integration/rc6-wave-a-core-20260914. Código permite sólo panel/cotización/histórico por los métodos IOL permitidos; estado de respuesta no vacía es NONEMPTY_UNVALIDATED, nunca readiness. Superficie reducida, no frontera de seguridad Python. Sin red/credenciales en pruebas, runtime, persistencia ni conexión a readiness. Revisión estática independiente del head anterior 5e475238fcbd0b429d7ec34873c7a894f0e46715: sin bloqueantes del alcance; mejora futura: tomar observed_at luego de respuesta si se usará para freshness.
- Test run 34888457062 falló 1/10: el test de vacío reemplazaba el método del cliente después de inyectarlo, y el wrapper ya retenía el método. Corregido en commit de pruebas 3f634b5b0066e752cfd64b9be646a1adb00dff56 mediante respuesta configurable del fake. CI final run 34888608698 SUCCESS en head 3f634b5b0066e752cfd64b9be646a1adb00dff56; compileall y 10 pruebas offline. El run fallido se conserva como antecedente.
- WA-02: primer entregable debe ser proyección pura en memoria, procedencia por campo y detección de IDENTITY_INCOMPLETE/LEGACY_KEY_COLLISION; no llamar current_records() porque puede inicializar esquema/DDL. Si después lee SQLite, conexión mode=ro, query_only=ON, SELECT explícito, sin bootstrap ni store mutator. No emite READY. Mantener el key legacy y la identidad expandida separadas; falta congelar CanonicalIdentity v1.
