...
The public PPI login bundles show the exact web contract:
- base URL: https://api.portfoliopersonal.com/
- route: /api/Seguridad/Auth/Login
- JSON body: {usuario, clave}
- public headers: AuthorizedClient=191206, ClientKey=pp123456
- fingerprint header name: fp, populated from the non-HttpOnly fp cookie.

This probe performs at most one authentication POST. Credentials and the fp
value are read only inside the trusted host/browser profile and never printed.
No token/user/message values are emitted; only safe response shape is returned.
"""
...
def main()->int:
    ...
    with sync_playwright() as pw:
        ...
        page=ctx.pages[0] if ctx.pages else ctx.new_page(); page.goto(LOGIN_URL,wait_until="domcontentloaded",timeout=20000); page.wait_for_timeout(1200)
        fp=""
        for c in ctx.cookies():
            if c.get("name")==FP_COOKIE and c.get("value"):
                fp=c["value"]; break
        if not fp:
            ctx.close(); print(json.dumps({"status":"BLOCKED_AUTH_FP_COOKIE_MISSING","fp_cookie_present":False,"credentials_exposed":False,"request_count":0,"orders_visited":False,"real_orders_sent":0},sort_keys=True)); return 4
        result=page.evaluate(
            """async ({u,p,url,ac,ck,fp}) => {
              ...
              try {
                const r=await fetch(url,{method:'POST',mode:'cors',credentials:'omit',headers:{'Accept':'application/json, text/plain, */*','Content-Type':'application/json','AuthorizedClient':ac,'ClientKey':ck,'fp':fp},body:JSON.stringify({usuario:u,clave:p})});
                safe.http_status=r.status; safe.ok=r.ok;
                let data=null; try{data=await r.json()}catch(_){}
                if(data&&typeof data==='object'&&!Array.isArray(data)){
                  safe.json_object=true; safe.envelope_success=!!data.success;
                  ...
                  if(payload){
                    ...
                    safe.has_token=!!(payload.token||payload.accessToken); safe.twofa=!!payload.twoFAInfo; safe.change_password=!!payload.changePassword;
                  }
                }
              }catch(_){safe.network_error=true}
              return safe;
            }""",
            {"u":user,"p":password,"url":LOGIN_API,"ac":PUBLIC_AUTHORIZED_CLIENT,"ck":PUBLIC_CLIENT_KEY,"fp":fp},
        )
        ctx.close()
    ...
    out={"status":status,"http_status":code,"ok":bool(result.get("ok")),"json_object":bool(result.get("json_object")),"envelope_success":bool(result.get("envelope_success")),"envelope_status":result.get("envelope_status"),"payload_object":bool(result.get("payload_object")),"safe_payload_keys":result.get("safe_payload_keys",[])[:20],"has_token_shape":bool(result.get("has_token")),"twofa_shape":bool(result.get("twofa")),"change_password_shape":bool(result.get("change_password")),"fp_cookie_present":True,"fp_value_exposed":False,"credentials_exposed":False,"request_count":1,"orders_visited":False,"real_orders_sent":0}
