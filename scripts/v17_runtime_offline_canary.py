...
VERSION='v17-runtime-offline-canary-1'
...
def canary():
    ...
    try:
        ...
        ready=False; deadline=time.monotonic()+30
        query=("import sqlite3,json; c=sqlite3.connect('/app/data/paper_v17/observer_v17.db'); "
               "r=c.execute('select process_state,real_orders_sent from observer_state where id=1').fetchone(); "
               "print(json.dumps(r if r else []))")
        while time.monotonic()<deadline:
            checked=docker('exec',NAME,'python','-c',query,capture_output=True)
            if checked.returncode==0:
                values=json.loads(checked.stdout)
                if len(values)==2 and values[1]==0:
                    ready=True; report['runtime_observed_state']=values[0]; break
