"""Sanitized normalization of authenticated PPI GET/XHR contract evidence.
...
excluded. The browser/XHR collector is evidence-only and cannot route orders.
"""
...
def _currency(row):
    ...
    if isinstance(value,dict):
        # Keep provider description/symbol only; account or user fields are absent.
...
def instrumentos_operables(payload):
    ...
    if isinstance(rows,dict): rows=rows.get('payload')
    ...
    for row in rows:
        ...
        out.append({
            'instrument_id':row.get('itemId'),
            'ticker':ticker,
            'provider_name':row.get('nombre'),
            ...
            # deliberately omitted: cantidadDisponible (account-specific)
            'quantity_step':None,
            'price_tick':None,
...
def bond_technical(payload):
    ...
    if isinstance(row,dict) and 'payload' in row and isinstance(row.get('payload'),dict): row=row['payload']
    if not isinstance(row,dict): return {}
    # Fields are retained under provider semantics; no order-unit inference.
