...
test -d "$REPO/.git"
test -f "$REPO/.env"
for f in fg_critical_approval_gateway_rc6.py fm_critical_approval_unix_runtime_rc6.py fn_critical_github_proxy_rc6.py; do test -f "$REMOTE_STAGE/$f"; done
...
echo "OBSERVER_PRE=$obs"; echo "DASHBOARD_PRE=$dash"
...
state="$(observer_db_state)"; echo "OBSERVER_DB_PRE=$state"; test "$state" = 'ok|PRODUCTION_PAPER|0'
...
for legacy in porota_trading_bot porota_sandbox_engine porota_production_engine; do if sudo -n docker ps --format '{{.Names}}' | grep -Fxq "$legacy"; then echo "LEGACY_TELEGRAM_CONSUMER=$legacy"; exit 1; fi; done
...
command -v gh >/dev/null; gh auth status -h github.com >/dev/null 2>&1; gh api 'repos/mbalbo2023/Porota-trading/issues/40' >/dev/null
echo 'GH_HOST_AUTH=AVAILABLE_NOT_EXPOSED_TO_CONTAINER'
telegram_token="$(extract_env TELEGRAM_BOT_TOKEN)"; telegram_chat="$(extract_env TELEGRAM_CHAT_ID)"; test -n "$telegram_token" && test -n "$telegram_chat"
...
with urllib.request.urlopen(url, timeout=15) as r: obj=json.load(r)
...
for f in fg_critical_approval_gateway_rc6.py fm_critical_approval_unix_runtime_rc6.py fn_critical_github_proxy_rc6.py; do sudo -n install -m 0644 "$REMOTE_STAGE/$f" "$RELEASE/$f"; done
# Host broker must traverse this directory to read its 0400 capability file.
# Keep it private to the existing control-plane account rather than widening mode bits.
sudo -n install -d -o "$CONTROL_USER" -g "$CONTROL_GROUP" -m 0700 "$SECRETS"
...
sudo -n chown 1000:1000 "$SECRETS/critical_telegram.token" "$SECRETS/critical_telegram.chat"; sudo -n chmod 0400 "$SECRETS/critical_telegram.token" "$SECRETS/critical_telegram.chat"
...
printf '%s' "$capability" > /tmp/porota-broker-capability.host; printf '%s' "$capability" > /tmp/porota-broker-capability.token
...
rm -f /tmp/porota-broker-capability.host /tmp/porota-broker-capability.token; unset capability
# Fail before installing systemd units if ownership/traversal regresses.
sudo -n -u "$CONTROL_USER" test -r "$BROKER_HOST_KEY"
test "$(stat -c '%U:%G:%a' "$SECRETS")" = "$CONTROL_USER:$CONTROL_GROUP:700"
echo 'BROKER_LOCAL_CAPABILITY=GENERATED_VALUE_NOT_LOGGED'; echo 'BROKER_HOST_CAPABILITY_ACCESS=GREEN'
...
WantedBy=multi-user.target
UNIT
sudo -n install -m 0644 /tmp/porota-critical-github-proxy-rc6.service "$PROXY_UNIT"; sudo -n install -m 0644 /tmp/porota-critical-approval-rc6.service "$GATEWAY_UNIT"
rm -f /tmp/porota-critical-github-proxy-rc6.service /tmp/porota-critical-approval-rc6.service
installed=1; sudo -n systemctl daemon-reload; sudo -n systemctl enable --now porota-critical-github-proxy-rc6.service
for _ in $(seq 1 30); do [[ -S /run/porota-critical-approval-rc6/github.sock ]] && break; sleep 1; done
test -S /run/porota-critical-approval-rc6/github.sock; sudo -n systemctl enable --now porota-critical-approval-rc6.service

echo '=== POSTFLIGHT ==='; sleep 15
test "$(sudo -n systemctl is-active porota-critical-github-proxy-rc6.service)" = active; test "$(sudo -n systemctl is-active porota-critical-approval-rc6.service)" = active
running="$(sudo -n docker inspect -f '{{.State.Running}}|{{.RestartCount}}|{{.HostConfig.ReadonlyRootfs}}|{{.Config.Image}}|{{.Config.User}}' "$GATEWAY_CONTAINER")"; echo "CONTROL_PLANE_CONTAINER=$running"; test "$running" = "true|0|true|$LIVE_IMAGE|botuser"
mounts="$(sudo -n docker inspect -f '{{range .Mounts}}{{.Source}}=>{{.Destination}};{{end}}' "$GATEWAY_CONTAINER")"; case "$mounts" in *docker.sock*|*ppi_production*|*'/opt/porota-trading/.env'*) echo 'FORBIDDEN_MOUNT=RED'; exit 1;; esac; echo 'FORBIDDEN_MOUNT=NONE'
...
proxy_log="$(sudo -n journalctl -u porota-critical-github-proxy-rc6.service -n 80 --no-pager)"; gateway_log="$(sudo -n journalctl -u porota-critical-approval-rc6.service -n 120 --no-pager)"
printf '%s\n' "$proxy_log" | grep -q 'critical-github-broker: READY'; printf '%s\n' "$proxy_log" | grep -q 'auth=local-key'; printf '%s\n' "$gateway_log" | grep -q 'critical-approval-runtime: READY'
...
obs_post_id="$(sudo -n docker inspect -f '{{.Id}}' porota_production_observer)"; dash_post_id="$(sudo -n docker inspect -f '{{.Id}}' porota_production_dashboard)"; test "$obs_post_id" = "$obs_id"; test "$dash_post_id" = "$dash_id"
obs_post="$(sudo -n docker inspect -f '{{.State.Running}}|{{.RestartCount}}|{{.HostConfig.ReadonlyRootfs}}|{{.Config.Image}}|{{json .Config.Cmd}}' porota_production_observer)"; dash_post="$(sudo -n docker inspect -f '{{.State.Running}}|{{.RestartCount}}|{{.Config.Image}}' porota_production_dashboard)"; echo "OBSERVER_POST=$obs_post"; echo "DASHBOARD_POST=$dash_post"
case "$obs_post" in 'true|0|true|'*'bv_paper_runtime.py'*) ;; *) exit 1;; esac; case "$dash_post" in 'true|0|'*) ;; *) exit 1;; esac
state_post="$(observer_db_state)"; echo "OBSERVER_DB_POST=$state_post"; test "$state_post" = 'ok|PRODUCTION_PAPER|0'
echo 'REAL_ORDERS_SENT=0'; echo 'TRADING_RUNTIME_UNCHANGED=GREEN'; echo "CONTROL_PLANE_SOURCE_SHA=$SOURCE_SHA"; echo 'CONTROL_PLANE_PERMANENT_DEPLOY=GREEN'
sudo -n rm -rf "$REMOTE_STAGE"; trap - EXIT
