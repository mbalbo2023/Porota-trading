...
Fail-closed server runner. It owns no broker/network stack: API residual state is
read directly from SQLite; browser work runs as porotaadmin; validated PPI Web
history is persisted with explicit PPI_WEB_HISTORY provenance.
"""
...
ENABLED=ROOT/"ENABLED"; MANIFEST=ROOT/"residual.jsonl"; STATE_DB=ROOT/"state.sqlite3"; STATUS=ROOT/"status.json"; BATCH_ROOT=ROOT/"batches"
...
class RuntimeSQLiteStore:
    """Minimal write-capable store facade for the existing attempt ledger only."""
...
def log(event:str,**fields):
    print(json.dumps({"event":event,"at":datetime.now(timezone.utc).isoformat(),**fields},ensure_ascii=False,sort_keys=True),flush=True)
...
def api_writer_active()->bool:
    return subprocess.run(["systemctl","is-active","--quiet",API_UNIT],check=False).returncode==0
...
def ensure_manifest()->list[dict]:
    rows=load_from_runtime(OBSERVER_DB)
    ROOT.mkdir(parents=True,exist_ok=True); tmp=ROOT/".residual.jsonl.tmp"; write_jsonl(tmp,rows); os.replace(tmp,MANIFEST); return rows
...
def reauthenticate():
    rc,out=run_json_command(browser_command(REAUTH,"--profile",str(PROFILE),"--secret",str(SECRET),"--chrome",CHROME),150)
    status=str(out.get("status","UNKNOWN")); log("reauth",rc=rc,status=status,credentials_exposed=False,real_orders_sent=0)
...
def collect(tasks:list[dict],batch_index:int)->dict:
    ...
    if str(summary.get("auth_status"))=="BLOCKED_AUTH_SESSION_EXPIRED": reauthenticate(); rc,summary=run_json_command(cmd,max(180,50*len(tasks)))
...
def identity(row:dict)->tuple[str,str,str,str]:
    return tuple(str(row.get(k,"")).strip().upper() for k in ("symbol","instrument_type","market","settlement"))
...
def process_batch(state:ResidualState,tasks:list[dict],capture:dict):
    rows_by_key=best_rows_by_identity(capture); store=RuntimeSQLiteStore(OBSERVER_DB); today=datetime.now(timezone.utc).date(); date_from=today-timedelta(days=365)
    for task in tasks:
        rows=rows_by_key.get(identity(task),[])
        if not rows:
            state.finish(RUN_ID,task,"DONE_EMPTY",provider_rows=0,valid_rows=0,detail="PPI_WEB_NO_HISTORICAL_CAPTURE"); continue
        try:
            ...
            detail=f"source=PPI_WEB_HISTORY; rejected={result['rejected_rows']}; canonical_updates={result['canonical_updates']}; protected_by_precedence={result['protected_by_precedence']}"
...
def main()->int:
    safety_check(); rows=ensure_manifest(); manifest_hash=sha256_file(MANIFEST); state=ResidualState(STATE_DB,STATUS); state.seed(RUN_ID,manifest_hash,rows); recovered=state.recover_orphan_running(RUN_ID)
    log("runner_start",run_id=RUN_ID,residual=len(rows),recovered=recovered,api_writer_active=False,real_orders_sent=0)
    batch_index=0
    while True:
        ...
        try: process_batch(state,tasks,collect(tasks,batch_index))
            ...
            state.heartbeat(RUN_ID,status="RUNNING"); raise
    state.complete_if_terminal(RUN_ID); summary=state.publish(RUN_ID); log("runner_complete",**summary,real_orders_sent=0); return 0
