...
def family_match(desc,fam):
    d=(desc or '').upper()
    aliases={
      'CEDEARS':['CEDEAR'], 'BONOS':['BONO'], 'ON':['OBLIGACION','OBLIGACIONES','ON'],
      'OPCIONES':['OPCION'], 'FUTUROS':['FUTURO'], 'FCI':['FCI','FONDO']}
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        print('PPI_AUTH_DIRECT_DISCOVERY_CANARY')
        print('AUTH=' + ('AUTHENTICATED_TRUSTED_DEVICE' if auth else 'BLOCKED_AUTH_SESSION_EXPIRED'))
        if not auth: ctx.close(); return 20
        # Never print header values. Reuse only safe browser request headers in memory.
        ...
        cfg_status,cfg=get_json('/api/Cotizaciones/Alertas/GetConfig')
        types=extract_type_items(cfg)
        ...
        for fam,sym in TARGETS:
            found=None; source='-'; ep='-'; http=0
            tid=fam_type.get(fam)
            if tid is not None:
                http,data=get_json('/api/Cotizaciones/Item/Tradeable',{'tipo':tid})
                ep='/api/Cotizaciones/Item/Tradeable'
                for obj in rows(data):
                    m=item_match(obj,sym)
                    if m: found=m; source='TRADEABLE_BY_TYPE'; break
            if not found:
                # Bounded safe GET alternatives for the global search wrapper; values are never logged.
                for key in ('search','text','query','value','ticker'):
                    ...
                    if found: break
            if found:
                print(f"{fam}|{sym}|RESOLVE=YES|SOURCE={source}|ITEM={found['id']}|TYPE_ID={found.get('type_id') or tid}|HTTP={http}|ENDPOINT={ep}")
                resolved.append((fam,sym,found,tid))
            else:
                print(f'{fam}|{sym}|RESOLVE=NO|TYPE_ID={tid}|HTTP={http}|ENDPOINT={ep}')
        ...
        print(f'RESOLVED={len(resolved)}/6')
        print(f'HISTORY_OK={hist_ok}/{len(resolved)}')
