...
DROP_KEY_PARTS = ("cuenta","account","saldo","tenencia","disponible","comitente","cliente","documento","dni","cuit",
                  "email","mail","telefono","phone","token","password","passwd","cookie","authorization","secret","session",
                  "usuario","username","user_id")
SAFE_KEY_PARTS = ("ticker","simbolo","símbolo","especie","descripcion","descripción","instrumento","moneda","currency","plazo",
                  "dia","día","venc","tasa","tna","minimo","mínimo","minimum","maximo","máximo","maximum","multiplo","múltiplo",
...
def schema_shape(payload):
    if isinstance(payload, dict):
        return {"type":"object","keys":sorted(str(k) for k in payload.keys() if not any(x in str(k).lower() for x in DROP_KEY_PARTS))[:80]}
    if isinstance(payload, list):
        return {"type":"array","count":len(payload),"first_keys":sorted(str(k) for k in payload[0].keys())[:80] if payload and isinstance(payload[0],dict) else []}
...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ctx = pw.chromium.launch_persistent_context(user_data_dir=args.profile, executable_path=args.chrome, headless=True,
                locale="es-AR", timezone_id="America/Argentina/Buenos_Aires", viewport={"width":1440,"height":1000},
            ...
            def on_response(response):
                try:
                    if response.request.method.upper() != "GET" or response.status >= 400 or not any(x in response.url for x in TARGETS):
                        return
    ...
    print(json.dumps({"state":out["auth_status"],"jobs":jobs,"routes":len(out["routes"]),
                      ...
                      "real_orders_sent":0,"capture":str(target)}, ensure_ascii=False, sort_keys=True))
