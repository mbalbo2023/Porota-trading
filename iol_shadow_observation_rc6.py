...
def refresh(symbols: Iterable[str], call_tool: Callable[[str, dict], dict], *,
            root: Path | str | None = None, market: str = DEFAULT_MARKET,
    ...
    for raw in symbols:
        ...
        try:
            ...
            rows.append({
                ...
                "state": "READY" if quote else "UNAVAILABLE", "quote": quote,
                "asset_type": str((info or {}).get("type") or ""),
                "currency": str((info or {}).get("currency") or ""),
                "units_per_lot": (info or {}).get("units_per_lot"),
                "primary_comparison": compare_background_numeric(
                    primary_last, quote.get("last"), tolerance_pct=tolerance_pct),
    ...
    payload = {
        "schema_version": SCHEMA_VERSION, "refreshed_at": observed_at, "source": SOURCE,
        "mode": MODE, "decision_effect": DECISION_EFFECT, "live_decision_authority": False,
        "real_money_authorized": False, "symbols": rows,
    }
...
def collect(root: Path | str | None = None) -> dict:
    ...
    if payload.get("schema_version") != SCHEMA_VERSION:
        return {
            "source": SOURCE, "mode": MODE, "state": "UNAVAILABLE",
    ...
    return {
        ...
        "state": "READY" if payload.get("symbols") else "INSUFFICIENT_DATA",
        "decision_effect": DECISION_EFFECT, "live_decision_authority": False,
        "real_money_authorized": False, "refreshed_at": payload.get("refreshed_at"),
